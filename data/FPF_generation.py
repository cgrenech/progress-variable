...
fuel_composition = 'CH4:0.2, H2:0.6, NH3:0.2'

oxidant_composition = 'O2:0.21, N2:0.79'    #air
# oxidant_composition = 'O2:1.0, N2:0.0'    #oxyfuel
...
# equivalence_ratio = np.linspace(0.4, 1.8, 80)
equivalence_ratio = np.array([1.8])
...
mechanism = 'C3Mech_H2_NH3_CH4_NOx'
...
def freeFlameSimulation(gas, fuel, oxidant, phi, T_in, P_in):
    ...
    f.set_max_grid_points(f.domains[f.domain_index("flame")],1000)
...
# density, viscosity, thermal conductivity, specific heat Cp
density_all = []
viscosity_all = []
thermal_conductivity_all = []
cp_mass_all = []
...
for phi in equivalence_ratio:
    ...
    # Down-sample the flame solution to have a fixed number of points for each flame simulation
    indices = np.round(np.linspace(0, len(grid) - 1, N_points_grid)).astype(int)
    ...
    grid_all.extend(f.grid[indices])

    ## Mixture fraction computed along the flame
    mixture_fraction_along_flame_all.extend(Z_bilger[indices])

    ## Mixture fraction at the inlet boundary
    mixture_fraction_inlet_all.extend([Z_inlet] * len(indices))

    ## Temperature
    temperature_all.extend(f.T[indices])

    ## Density
    density_all.extend(f.density[indices])

    ##Thermal conductivity 
    thermal_conductivity_all.extend(f.thermal_conductivity[indices])

    ##Viscosity 
    viscosity_all.extend(f.viscosity[indices])

    ##Specific_heat
    cp_mass_all.extend(f.cp_mass[indices])

    ## Species mass fraction
    for i, species in enumerate(gas.species_names):
        mass_fraction_all[f"Mass fraction of {species}"].extend(f.Y[i][indices])

    ## Temperature source term
    temperature_source_term.extend(f.heat_release_rate[indices]/f.cv_mass[indices])
...
results_grid.to_csv("FPF-"+ data_tag +"-grid.csv", index=False, header=False)
...
results_mixture_fraction_inlet.to_csv("FPF-"+ data_tag +"-mixture-fraction.csv", index=False, header=False)
...
results_mixture_fraction_along_flame.to_csv("FPF-"+ data_tag +"-mixture-fraction-along-flame.csv", index=False, header=False)

# Save temperature and species mass fractions
results_state_space = {
    "Temperature": temperature_all, 
    "Density": density_all,
    "Viscosity": viscosity_all,
    "Thermal conductivity" : thermal_conductivity_all,
    "Specific heat": cp_mass_all,
    }
results_state_space.update(mass_fraction_all)
results_state_space = pd.DataFrame(results_state_space)
results_state_space.to_csv("FPF-"+ data_tag +"-state-space.csv", index=False, header=False)
...
results_state_space_sources.to_csv("FPF-"+ data_tag +"-state-space-sources.csv", index=False, header=False)


# print("Species names:", gas.species_names)
print(gas.n_total_species)
